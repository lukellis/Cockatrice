 ...
 * "{digit}" or "{W|U|B|R|G|C}" tokens (case-insensitive) with nothing left over -- a hybrid
 * ("{R/G}"), Phyrexian ("{R/P}"), variable ("{X}"), snow ("{S}"), or split-cost ("3U // 4UU")
 * symbol anywhere in the string means the whole cost is not confidently parseable, so this returns
 * std::nullopt rather than guess at a partial cost (same "exact shape or skip" conservatism as
 * every other parser in this fork). An empty string (no printed mana cost -- true of every basic
 * land) parses to a free ManaCost{} : zero tokens trivially covers the whole (empty) string.
 */
